...
# High enough that you'll never run out of "manager" slots.
# 5,000 threads is well within limits of modern OSs for sleeping threads.
...
def run_parallel_or_seq(
    ...
    *task_args: Any,
    ...
    """Run task_fn over items in parallel (semaphore-limited) or sequentially."""
    results: list[Any] = []
    ...
    if parallel:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc=cdesc,
                           miniters=1, smoothing=0.1, unit="file"):
            item = futures[future]
            try:
